...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const {jsPDF} = window.jspdf || { };
    ...
    // ---- Phase config ----
    const PHASES = [
    {key: "idea", title: "Idea and POC", fields: [
    ...
    {key: "planning", title: "Planning and Funding", fields: [
    ...
    {key: "marketing", title: "Marketing and Launch", fields: [
    ...
    {key: "postmortem", title: "Post Mortem and Improve", fields: [
    ...
    {key: "refine", title: "Refine and Dial In", fields: [
  ...
  const saveState = (s) => localStorage.setItem(STORAGE_KEY, JSON.stringify(s));
    ...
    // If the Phase tab isn't present on this page, bail quietly.
    if (!phaseSelect || !phaseFields || !phaseMvp) return;

    // ---- State ----
    let appState = loadState();
    let currentKey = phaseSelect.value || "idea";
...
`;
    ...
    // Resources (if any)
    const res = PHASES.find(p => p.key === currentKey)?.resources;
    if (res && res.length && phaseResources) {
      phaseResources.innerHTML = `
    < div >
          <h4 class="text-sm font-semibold mb-2">Helpful Resources</h4>
          <ul class="list-disc pl-5 space-y-1">
            ${res.map(r => `<li><a class="text-indigo-400 hover:underline" target="_blank" rel="noreferrer" href="${r.href}">${r.label}</a></li>`).join("")}
          </ul>
        </div > `;
    ...
    if (phasePdfBtn) phasePdfBtn.classList.toggle("hidden", currentKey !== "refine");
  }

  // ---- Prev/Next helpers ----
  function currentIndex() { return PHASES.findIndex(p => p.key === currentKey); }
  function setIndex(i) {
  ...
  // ---- PDF generator (single implementation) ----
  function generatePdf() {
    ...
    const addHeader = (text) => { doc.setFont("helvetica","bold"); doc.setFontSize(18); doc.text(text, marginX, y); y += sectionGap; };
    const addSub    = (text) => { doc.setFont("helvetica","bold"); doc.setFontSize(13); doc.text(text, marginX, y); y += titleGap; };
    const addText   = (label, value) => {
      doc.setFont("helvetica","bold"); doc.setFontSize(11); doc.text(label, marginX, y); y += 14;
      doc.setFont("helvetica","normal"); doc.setFontSize(11);
      const maxWidth = doc.internal.pageSize.getWidth() - marginX * 2;
      doc.splitTextToSize(value || "—", maxWidth).forEach(ln => { doc.text(ln, marginX, y); y += lineGap; });
      y += 6;
    };
    const addMvp = (isMvp) => { doc.setFont("helvetica","bold"); doc.setFontSize(10); doc.text(`MVP ? ${ isMvp ? "Yes" : "No" } `, marginX, y); y += sectionGap - 6; };
    const ensure = (needed=120) => { const H = doc.internal.pageSize.getHeight(); if (y + needed > H - marginTop) { doc.addPage(); y = marginTop; } };

    // Cover
    doc.setFont("helvetica","bold"); doc.setFontSize(22); doc.text("Project Phases — Draft", marginX, y); y += 28;
    doc.setFont("helvetica","normal"); doc.setFontSize(12); doc.text(`Generated: ${ new Date().toLocaleString() } `, marginX, y); y += 40;

    // Each phase
    PHASES.forEach((phase, idx) => {
      ensure(); addHeader(`${ idx + 1 }. ${ phase.title } `); addMvp(!!appState[phase.key].mvp);
      phase.fields.forEach(f => { ensure(90); addText(f.label, appState[phase.key].values[f.key]); });
      if (phase.resources?.length) {
        ensure(80); addSub("Resources"); doc.setFont("helvetica","normal"); doc.setFontSize(11);
        phase.resources.forEach(r => { ensure(30); doc.text(`• ${ r.label }: ${ r.href } `, marginX, y); y += lineGap; }); y += 8;
      }
    });
